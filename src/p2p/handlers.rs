...
mod tests {
    ...
    use crate::piece_picker::{PickResult, MockPiecePicker, PiecePicker};
    ...
    async fn pick_blocks_test() {
        ...
        let pick_result = PickResult {
            blocks: vec![Block::new(0, 0, 0)],
            end_game_mode_enabled: false,
        };
        let mut pick_option = Some(pick_result.clone());
        let mut picker = MockPiecePicker::new();
        picker.expect_pick().returning(move |_, _| pick_option.take());
    ...
    #[tokio::test]
    async fn pick_blocks_when_end_game_picked_by_picker_test() {
        let mut result = HandlerResult::new();
        let mut state = P2PState::new(0, Bitfield::init(5), 5);
        state.client_is_interested = true;
        state.client_is_choked = false;
        ...
        assert!(result.internal_events[0].is_end_game_enabled());
        assert_eq!(state.ongoing_requests.len(), pick_result.blocks.len());
        assert_eq!(result.messages_for_peer.len(), pick_result.blocks.len());
        assert!(result.messages_for_peer.iter().all(|msg| msg.is_request()));
        assert_eq!(state.ongoing_requests.len(), pick_result.blocks.len());
    ...
    #[tokio::test]
    async fn pick_blocks_when_no_pieces_picked_by_picker_test() {
        let mut result = HandlerResult::new();
        let mut state = P2PState::new(0, Bitfield::init(5), 5);
        state.client_is_interested = true;
        state.client_is_choked = false;
        ...
        let mut picker = MockPiecePicker::new();
        picker.expect_pick().returning(move |_, _| None);
        let picker = Arc::new(Mutex::new(picker));
